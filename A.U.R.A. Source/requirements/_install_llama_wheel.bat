...
if /i "%AURA_LLAMA_WHEEL%"=="cuda" (
    ...
    "%PYTHON_EXE%" -c "import sys; sys.path.insert(0, r'%SOURCE_DIR%'); import bootstrap_llama; ok,_=bootstrap_llama.probe_llama_backend(require_cuda=True); sys.exit(0 if ok else 1)"
...
if /i "%AURA_LLAMA_WHEEL%"=="vulkan" (
    ...
    "%PYTHON_EXE%" -c "import sys; sys.path.insert(0, r'%SOURCE_DIR%'); import bootstrap_llama; ok,_=bootstrap_llama.probe_llama_backend(require_vulkan=True); sys.exit(0 if ok else 1)"
    if %ERRORLEVEL% EQU 0 exit /b 0
